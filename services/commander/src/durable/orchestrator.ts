...
const decisionCostAttributions = (decision: PlannedDecision): readonly CostAttribution[] =>
  decision.costAttributions ?? [
    {
      model: decision.model,
      tokenUsage: decision.tokenUsage,
      costUsd: decision.costUsd,
  ...
  readConfigEffect(env).pipe(
    Effect.flatMap((config) => {
      ...
      return Effect.tryPromise({
        try: (signal) =>
          fetch(`${seat.endpoint.replace(/\/$/, "")}/healthz`, {
            headers,
            signal,
          }),
        catch: (cause) => cause,
...
export class OrchestratorAgent extends Agent<Env, CommanderSessionState> {
  ...
  override validateStateChange(nextState: CommanderSessionState): void {
    try {
      ...
      throw new Error(`Commander session state failed schema validation: ${detail}`, {
        cause,
      });
    }
  }
  ...
  override onStart(): Promise<void> {
    return Effect.runPromise(
      Effect.gen({ self: this }, function* () {
        yield* Effect.all([this.decisionLogs().initialize, this.sessionArchive().initialize]);
  ...
  override onMessage(connection: Connection, raw: WSMessage): Promise<void> {
    return Effect.runPromise(
      this.handleSeatMessage(connection, raw).pipe(
        Effect.catch((cause) =>
          Effect.sync(() => {
            connection.send(
              seatMessage({
        ...
        this.setState({
          ...
          memory: {
            ...
            shortTerm: {
              ...
              decisions: [...this.state.memory.shortTerm.decisions, { timestamp, summary }].slice(
                -30,
              ),
            },
          },
  ...
  handleTick(request: TickRequest): Promise<TickResponse> {
    return Effect.runPromise(
      Effect.gen({ self: this }, function* () {
        ...
        yield* Effect.forEach(drained, (result) =>
          result._tag === "Failure"
            ? Effect.logWarning(
        ...
        yield* Effect.forEach(assessments, (assessment) =>
          this.decisionLogs().save(assessment.log),
        );
        next = this.state;
            ...
            memory: {
              ...
              shortTerm: {
                ...
                decisions: [
                  ...
                  .filter((item) => item.timestamp >= request.timestamp - SHORT_TERM_WINDOW_SECONDS)
                  .slice(-30),
              },
            },
          };
          ...
          (report) =>
            Effect.tryPromise({
              try: async () => {
                ...
                repositoryFailure(`sergeant ${report.payload.group_id} scheduling`, cause),
        ...
        const strategicReport = request.sergeant_reports.find(
          (report) =>
  ...
  private enqueueDecisionRun(version: number): Effect.Effect<void, Error> {
    return Effect.tryPromise({
      try: () =>
        this.schedule(0, "runScheduledDecision", { kind: "commander", version } as const, {
          idempotent: true,
  ...
  registerSeat(input: LlmSeatRegistrationRequest): Promise<SeatRegistration[]> {
    return Effect.runPromise(
      Effect.gen({ self: this }, function* () {
        ...
        const healthy =
          current.mode === "contributor" ? false : yield* probeHttpSeat(current, this.env);
        ...
        const seats = [...this.state.seats.filter((item) => item.id !== seat.id), seat].sort(
    ...
    return Effect.runPromise(
      Effect.gen({ self: this }, function* () {
        ...
        const expiredJobIds = this.state.contributorJobLedger
          ...
          .map((job) => job.id);
        const seats = yield* Effect.forEach(
          reclaimed.seats,
          (candidate) => {
            ...
            if (seat.mode === "contributor") {
              return Effect.succeed<SeatRegistration>({
                ...
                healthy:
                  seat.healthy && seat.healthExpiresAt !== undefined && seat.healthExpiresAt > now,
              });
            }
  ...
  chargeSeatUsage(seatId: string, costUsd: number): Promise<SeatRegistration[]> {
    return Effect.runPromise(
      Effect.sync(() => {
        const seats = [
          ...reconcileSeatBudgetState(this.state.seats, seatId, 0, costUsd, utcBudgetPeriod()),
        ];
        this.setState({ ...this.state, seats });
        return seats;
      }),
  ...
  reserveSeatBudget(
    ...
    return Effect.runPromise(
      Effect.sync(() => {
        ...
        const reservations =
          reservationId === undefined || !mutation.accepted
  ...
  private reclaimStaleReservations(
    ...
    const stale = state.seatBudgetReservations.filter(
      (reservation) =>
  ...
  markSeatUnhealthy(seatId: string): Promise<readonly SeatRegistration[]> {
    return Effect.runPromise(
      Effect.sync(() => {
        const seats = this.state.seats.map(
  ...
  invokeContributor(
    ...
    return Effect.runPromise(
      Effect.gen({ self: this }, function* () {
        const jobId =
          jobKey === undefined ? `job_${crypto.randomUUID()}` : contributorJobId(seatId, jobKey);
        ...
        const seat = this.state.seats.find(
          (item) => item.id === seatId && item.mode === "contributor",
        );
        ...
        const connection = [...this.getConnections<SeatConnectionState>()].find(
          (candidate) =>
            candidate.id === seat.activeConnectionId && candidate.state?.seatId === seatId,
        );
        ...
        yield* Effect.sync(() =>
          this.setState({
            ...
            seatBudgetReservations: this.state.seatBudgetReservations.map((reservation) =>
              reservation.id === durableLeaseId ? { ...reservation, jobId } : reservation,
            ),
          }),
        );
        ...
        const awaiting = Effect.callback<AiDecisionResult, Error>((resume) => {
          ...
          Effect.timeoutOrElse({
            ...
            orElse: () => {
              const timeout = timeoutContributorError(jobId);
              return Effect.sync(() =>
                this.terminalizeContributorJob(jobId, timeout, Date.now() / 1_000),
  ...
  async invokeContributorOutcome(
    ...
    try {
      return {
        ok: true,
        result: await this.invokeContributor(seatId, tier, prompt, timeoutSeconds, jobKey, leaseId),
      };
    } catch (cause) {
  ...
  private handleSeatMessage(connection: Connection, raw: WSMessage): Effect.Effect<void, unknown> {
    return Effect.gen({ self: this }, function* () {
      ...
      if (message.type === "register") {
        ...
        yield* Effect.sync(() => {
          ...
          for (const candidate of this.getConnections<SeatConnectionState>()) {
            if (candidate.id !== connection.id && candidate.state?.seatId === seat.id)
          ...
          (seat): SeatRegistration =>
            seat.id !== message.seat_id
              ...
              : {
                  ...
                  ...(message.queue_depth === undefined ? {} : { queueDepth: message.queue_depth }),
                },
        );
        yield* Effect.sync(() => {
          this.setState({ ...this.state, seats });
          connection.send(
            seatMessage({
              ...
              expires_at: new Date((now + config.seatHeartbeatTtlSeconds) * 1_000).toISOString(),
            }),
          );
        });
        return;
      ...
      let persistedJob = this.state.contributorJobLedger.find((item) => item.id === message.job_id);
      ...
      if (persistedJob?.status === "pending" && persistedJob.deadlineAt <= now) {
        ...
        liveJob = undefined;
        persistedJob = this.state.contributorJobLedger.find((item) => item.id === message.job_id);
      }
      ...
      if (persistedJob !== undefined && persistedJob.status !== "pending") {
        if (liveJob !== undefined) {
          const terminal =
            persistedJob.status === "succeeded" && persistedJob.result !== undefined
        ...
        if (
          ...
          yield* Effect.sync(() =>
            this.accountLateContributorUsage(
              message.job_id,
              tokenUsage,
              costUsd,
      ...
      if (message.ok) {
        ...
        const costUsd = message.usage.estimated_cost_usd ?? estimateCost(jobTier, tokenUsage);
        const decoded = yield* Effect.result(decodeContributorDecision(message.decision));
        ...
        yield* Effect.sync(() => {
          ...
          const seats = message.exhausted
            ? this.state.seats.map((seat) =>
                seat.id === message.seat_id ? { ...seat, healthy: false, exhausted: true } : seat,
              )
            : this.state.seats;
          this.setState({ ...this.state, seats, contributorJobLedger });
          this.contributorJobs.delete(message.job_id);
  ...
  persistSessionExport(exportId?: string): Promise<SessionExportMetadata> {
    return Effect.runPromise(
      Effect.gen({ self: this }, function* () {
        ...
        const options = exportId === undefined ? { exportedAt } : { exportedAt, id: exportId };
        ...
        for (
          ...
          descriptors.push(
            yield* repository.writePage(begun.lease, index, {
