...
import { CommanderApi, MachineAuth } from "./contract";
...
const readCachedMapBriefing = (
  kv: KVNamespace,
  mapName: string,
): Effect.Effect<MapBriefingType | undefined> =>
  Effect.gen(function* () {
    ...
    return provenanceMatches && terrainCacheKey(briefing) === pointer.key ? briefing : undefined;
  });
...
const webRequest = (request: HttpServerRequest.HttpServerRequest) =>
      ...
      withMachineAuth(
        ...
        Effect.gen(function* () {
          ...
          const result = yield* rpc<void>("disconnect", () =>
            stub.disconnectSession(payload.reason),
          );
            ...
            try: () =>
              Promise.all([
                ...
                ...(payload.briefing.source === "simulator_synthetic"
                  ? []
                  : [env.TERRAIN_CACHE.put(latestKey, JSON.stringify(pointer))]),
              ]),
            catch: (cause) => cause,
          }).pipe(Effect.result);
    ...
    .handle("listSessionExports", ({ query, request }) =>
      withAccess(request, "read", () =>
        Effect.gen(function* () {
          ...
          return listed._tag === "Success" ? listed.success : internalFailure(listed.failure);
        }),
      ),
    )
  ...
  Effect.gen(function* () {
    ...
    if (!env.SEAT_REGISTRATION_TOKEN && !env.STAVKA_SEAT_KEYS) {
      return errorResponse(
        503,
        "MISCONFIGURED",
        "Seat registration credentials are not configured",
      );
    }
...
const MethodFallbacks = Layer.mergeAll(
  HttpRouter.add("*", "/api/*", () =>
    Effect.sync(() => errorResponse(405, "METHOD_NOT_ALLOWED", "Unsupported API method")),
  ),
  HttpRouter.add("*", "/admin/*", () =>
    Effect.sync(() => errorResponse(405, "METHOD_NOT_ALLOWED", "Unsupported admin method")),
  ),
);
