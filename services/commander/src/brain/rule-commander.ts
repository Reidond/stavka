...
const cellPosition = (
  ...
  return [(grid[0] + 0.5) * resolution, elevation, (grid[1] + 0.5) * resolution];
};
...
const traversablePosition = (
  ...
  const grid = [Math.floor(desired[0] / resolution), Math.floor(desired[2] / resolution)] as const;
  const desiredCell = briefing.terrain_grid.find(
    (cell) => cell.grid[0] === grid[0] && cell.grid[1] === grid[1],
  );
  if (
  ...
  const featureCells = briefing.key_features
    ...
    .map((feature) => ({
      feature,
      cell: briefing.terrain_grid.find(
        (cell) => cell.grid[0] === feature.grid[0] && cell.grid[1] === feature.grid[1],
      ),
    }))
...
const spawnPoint = (state: CommanderSessionState, snapshot: GameSnapshot): Position | undefined =>
  traversablePosition(
    state,
...
export const planRuleCommander = (state: CommanderSessionState, trigger: string): RulePlan => {
  ...
  const idle = snapshot.friendly_groups.find(
    (group) => group.status === "idle" || group.status === "defending",
  );
  ...
  return {
    ...
    vehiclesReserved: commands.some(
      (command) =>
