...
const DEFAULT_LOCAL_PUBLIC_ADDRESS: &str = "127.0.0.1";
...
fn set_arma_public_address(config_path: &Path, public_address: &str) -> Result<String> {
...
impl TailscaleIntegration {
    pub async fn setup_and_start(
        ...
        println!("Updated publicAddress: {} -> {}", old_address, tailscale_ip);
        println!("ACL policy updated with server and player tags");

        Ok(TailscaleSession { tailscale_ip })
    }
    ...
    pub async fn cleanup_after_stop(
        ...
        let _ = set_arma_public_address(config_path, DEFAULT_LOCAL_PUBLIC_ADDRESS)?;
        println!(
            "Restored publicAddress to {} and cleaned Tailscale automation state",
            DEFAULT_LOCAL_PUBLIC_ADDRESS
        );
        Ok(())
    }

    pub fn format_connection_info(ip: &str, port: u16) -> String {
        let mut info = String::new();
        info.push_str("\n=== Arma Server Connection Info ===\n\n");
        info.push_str(&format!("Server Address: {}: {}\n", ip, port));
        info.push_str(&format!("Direct Connect: {}:{}", ip, port));
