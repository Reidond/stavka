...
| Hosted Maskirovka gateway      | PRODUCT Posture A: Worker + Container with Claude+Codex in-process; DO persistence; browser `/admin/auth` credential store; Access-gated `/_/`; R2 replay binding; machine bearer on `/healthz`/`/v1/*`                                                                               | Public workers.dev HTTP (account 1042 blocker), Access apps, live subscription tokens |
| Hosted Maskirovka leaf         | Optional single-seat Cloudflare Container (`apps/maskirovka-seat`); not personal-machine dial-in. Machine-only model routes; FIFO; auth checkpoint; Access dashboard                                                                                                                  | Optional experiment only — gateway is the supported hosted posture                    |
