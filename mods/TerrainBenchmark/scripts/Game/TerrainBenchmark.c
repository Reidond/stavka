modded class SCR_BaseGameMode {
  ...
  protected void RunTerrainBenchmark() {
    ...
    Print(string.Format("Map bounds: (%1, %2) to (%3, %4)", mins[0], mins[2],
                        maxs[0], maxs[2]),
          LogLevel.NORMAL);
    Print(string.Format("Map size: %1m x %2m", mapSizeX, mapSizeZ),
          LogLevel.NORMAL);
    Print("----------------------------------------", LogLevel.NORMAL);
  ...
  protected void BenchmarkGrid(BaseWorld world, string label, float step,
                               float startX, float startZ, float sizeX,
                               float sizeZ) {
    ...
    Print(string.Format("[%1]  %2 x %3 = %4 samples", label, nx, nz, total),
          LogLevel.NORMAL);
    Print(string.Format("  Time: %1 ms  |  %2 samples/ms", elapsed, rate),
          LogLevel.NORMAL);
    Print(string.Format("  Heights: min=%1  max=%2  avg=%3", hMin, hMax, hAvg),
          LogLevel.NORMAL);
    Print("----------------------------------------", LogLevel.NORMAL);
  }
}
