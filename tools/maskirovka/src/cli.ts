...
const configWithArguments = (forceLocal = false): Effect.Effect<
  MaskirovkaConfig,
...
const printDoctor = (
  config: MaskirovkaConfig,
  write = true,
): Effect.Effect<boolean, GatewayError> => Effect.gen(function*() {
...
const contributorSeatId = (provider: "claude" | "codex"): string => {
  const machine = hostname().toLowerCase().replace(/[^a-z0-9._:-]+/gu, "-").slice(0, 96);
...
const runContributor = (
  ...
  endpoint: string,
  token: string,
): Effect.Effect<never, unknown> => Effect.gen(function*() {
  ...
  if (selectedProvider !== undefined && selectedProvider !== "claude" && selectedProvider !== "codex") {
    return yield* Effect.fail(new Error("--provider must be claude or codex"));
  }
  const providers = (["claude", "codex"] as const).filter((provider) =>
    selectedProvider === undefined || selectedProvider === provider);
  ...
  if (explicitSeatId !== undefined && (
    explicitSeatId.length > 128 || !/^[a-zA-Z0-9][a-zA-Z0-9._:-]*$/u.test(explicitSeatId)
  )) {
    return yield* Effect.fail(new Error("--seat-id must be a valid contributor protocol seat id"));
  }
  const codexWorkspace = join(config.stateDirectory, "codex-workspace");
  ...
  Effect.gen(function*() {
    ...
    if (command === "serve" && hasFlag("--register")) {
      ...
      if (endpoint === undefined || token === undefined) {
        return yield* Effect.fail(new Error(
          "serve --register requires a WebSocket URL and --token <registration-token>",
        ));
      }
...
const runSmoke = (): Effect.Effect<void, unknown> => Effect.scoped(Effect.gen(function*() {
  ...
  const responses = yield* withWebHandler(app, (handler) => Effect.all([
    ...
    Effect.tryPromise(() => handler(new Request("http://maskirovka.local/v1/responses", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(sampleBody),
    }))),
    Effect.tryPromise(() => handler(new Request("http://maskirovka.local/v1/messages?beta=true", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
...
const runEval = (): Effect.Effect<void, unknown> => Effect.gen(function*() {
  const base = yield* configWithArguments();
  if (!hasFlag("--replay") && base.mode !== "replay") {
    return yield* Effect.fail(new Error(
      "eval requires --replay; record/live evals are deliberate operator actions",
    ));
  }
  let networkCalls = 0;
...
const runDeploySeat = (): Effect.Effect<void, GatewayError> => Effect.gen(function*() {
