...
import { Clock, Data, Deferred, Duration, Effect, Queue, Ref, Schedule, Schema } from "effect";
...
const decisionJsonSchema = (
  Object.keys(decisionDocument.definitions).length === 0
...
export class ContributorConnectionError extends Data.TaggedError("ContributorConnectionError")<{
...
const connectionFailure = (
  ...
  new ContributorConnectionError({
    code,
    message,
...
const decodeDecision = (result: SeatResult): Effect.Effect<ContributorDecision, GatewayError> =>
...
): Effect.Effect<never, ContributorConnectionError> =>
  Effect.gen(function* () {
    const tracker =
      options.tracker ??
      new WindowTracker({
    ...
    const governor = new FairGovernor(
      options.concurrency ?? (options.provider === "claude" ? 2 : 1),
    );
    ...
    const connectOnce = Effect.scoped(
      Effect.gen(function* () {
        ...
        const socket = yield* Socket.fromWebSocket(socketAcquire(options.endpoint, options.token), {
        ...
        const encode = (
          message: LlmContributorClientMessage,
        ): Effect.Effect<string, ContributorConnectionError> =>
        ...
        const send = (
          message: LlmContributorClientMessage,
        ): Effect.Effect<void, ContributorConnectionError> =>
          encode(message).pipe(
            Effect.flatMap((encoded) => Queue.offer(outbound, encoded)),
            Effect.asVoid,
          );

        const status = (): Effect.Effect<ContributorStatus> =>
          Effect.sync(() =>
        ...
        const heartbeat = Effect.gen(function* () {
          yield* Deferred.await(registered).pipe(
        ...
        const invoke = (
          ...
          Effect.gen(function* () {
            const failure = yield* Effect.result(
              Effect.gen(function* () {
                ...
                const expectedDialect =
                  options.provider === "claude" ? "anthropic-messages" : "openai-responses";
                ...
                const completed = yield* governor
                  .run(
                    Effect.acquireUseRelease(
                      ...
                      (reservation) =>
                        options.adapter.invoke(request).pipe(
                          Effect.flatMap((result) => {
                            ...
                            const planCreditUsd =
                              result.usage.planCreditUsd ??
                              estimateApiListCost(message.invocation.tier, usage);
                          ...
                          Effect.catch((error) => {
                            if (
                              ...
                              error.code === "INVALID_SEAT_RESPONSE"
                            ) {
                            ...
                            const usage = {
                              ...
                              outputTokens: Math.max(
                                0,
                                Math.floor(error.providerUsage.outputTokens),
                              ),
                ...
                if (
                  ...
                  yield* Effect.logWarning(
                    `Contributor seat ${options.id} exhausted its configured budget`,
                  );
                }
              }),
            );
            ...
            yield* send({
              ...
              exhausted:
                encoded.code === "SEAT_BUDGET_EXHAUSTED" ||
        ...
        const receive = (raw: string): Effect.Effect<void, ContributorConnectionError> =>
          Schema.decodeUnknownEffect(Schema.fromJsonString(LlmContributorServerMessage), {
        ...
        const writer = Queue.take(outbound).pipe(
          ...
          Effect.forever,
          Effect.mapError((cause) =>
            connectionFailure("SOCKET_WRITE_FAILED", "Contributor WebSocket write failed", cause),
          ),
        );
        ...
        const reader = socket
          ...
          .pipe(
            Effect.mapError((cause) =>
              cause instanceof ContributorConnectionError
                ? cause
                : connectionFailure(
                    "SOCKET_READ_FAILED",
      ...
      Effect.andThen(
        Effect.fail(connectionFailure("SOCKET_DISCONNECTED", "Contributor WebSocket disconnected")),
      ),
    );
